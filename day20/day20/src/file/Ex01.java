...
//		System.out.println("입력한 글자는"+ch);
		...
		File f1 = new File("a.txt");				//파일이름만 문자열로 전달 ( 상대경로 )
		File f2 = new File("c:\\20221110");			// 폴더 및 파일의 절대경로를 무자열로 전달
		File f3 = new File(f2,"b.txt");				// 부모 경로를 파일로, 파일이름은 문자열로 전달
		File f4 =new File ("C:\\20221110","c.txt");	// 부모 경로를 문자열로 ,파일의 이름은 문자열로 전달
		
		
//		자바에서 위치와 이름을 지정하여 파일 객체를 생성해도, 실제 파일이 만들어지는 것은 아니다
//		자바에서 파일 객체 생성하는 작업은 실제 파일의 존재와 상관이 없다
		
		f1.createNewFile();	//비어있는 새로운 파일을 생성한다 ( 생성 여부를 boolean으로 반환)
		f2.mkdir();			//폴더(디렉토리)를 생성한다  ( 생성 여부를 boolean으로 반환)
		f3.createNewFile();
		f4.createNewFile();
		
		System.out.println("f2의 절대 경로 : " + f3.getAbsolutePath());
		System.out.println("f3의 이름 : " + f3.getName());
		System.out.println("f3의 파일크기 (byte)"+f3.length());
		//형식
		...
		String[] list = f2.list();	// String 형식의 리스트
		for(String str : list) {
